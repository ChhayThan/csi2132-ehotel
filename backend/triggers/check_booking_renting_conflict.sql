...
$$
    ...
    BEGIN
        SELECT booking_conflicts, renting_conflicts INTO conflicting_booking_count, conflicting_renting_count
        FROM num_conflicts(NEW.checkin_date, NEW.checkout_date)
        WHERE hid = NEW.hid AND room_number = NEW.room_number;

        IF conflicting_booking_count > 1 THEN
...
$$
    ...
    BEGIN
        ...
        IF conflicting_renting_count > 1 THEN
            RAISE unique_violation USING MESSAGE = 'Conflicting renting for room in date range';
        END IF;
